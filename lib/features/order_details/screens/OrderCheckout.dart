...
class OrderNormalCheckout extends StatefulWidget {
  const OrderNormalCheckout({super.key, required this.itemTotalAmount, required this.discount, this.eeDiscount, required this.tax, required this.shippingCost, required this.id, required this.total, required this.product, required this.orderDetailsModel, });
 ...
 final double? eeDiscount;
  ...
  @override
  State<OrderNormalCheckout> createState() => _OrderNormalCheckoutState();
}

class _OrderNormalCheckoutState extends State<OrderNormalCheckout> {
  ...
  void initState() {
    ...
    Provider.of<PaymentController>(context,listen: false).getType('order');
    Provider.of<PaymentController>(context,listen: false).getOrderId(widget.id!.toString());
     Provider.of<PaymentController>(context,listen: false).getAmount(widget.total);

     Provider.of<PaymentController>(context,listen: false).getPaymentMethod(context,'order');
...
const SizedBox(height: 10,)
              ...
              child: Consumer<CheckoutController>(
                  builder: (context, checkoutController, child) {
                    ...
                    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      AmountWidget(title: '${getTranslated('sub_total', context)} ${' (${widget.product.length} ${getTranslated('items', context)}) '}',
                          amount: PriceConverter.convertPrice(context, widget.itemTotalAmount)),
                      AmountWidget(title: getTranslated('shipping_fee', context),
                          amount: PriceConverter.convertPrice(context, widget.shippingCost)),
                      ...
                      AmountWidget(title: getTranslated('Shipping_tax', context),
                          amount: PriceConverter.convertPrice(context, widget.shippingCost*0.15  )),
                      ...
                      AmountWidget(title: getTranslated('total_payable', context),
                          amount: PriceConverter.convertPrice(context,
                              (widget.itemTotalAmount +widget. shippingCost - widget.discount - couponDiscount + widget.tax+(widget.shippingCost*0.15)))),
                    ]);})),
          const SizedBox(height: 10,),
          ...
          if(Provider.of<AuthController>(context, listen: false).isLoggedIn())
            Padding(padding: const EdgeInsets.only(bottom: Dimensions.paddingSizeSmall),
                child: CouponApplyWidget(couponController: couponController, orderAmount: (widget.itemTotalAmount + widget.shippingCost - widget.discount - couponDiscount + widget.tax))),
          ...
          Consumer<SyncOrderController>(
            ...
            return 0;
                  ...
                  int addressID=    checkout.addressIndex??0;
                      ...
                      orderAmount: (widget.itemTotalAmount + widget.shippingCost - widget.discount - couponDiscount + widget.tax),
                      onTap: ()async{if(profileProvider.balance! <
                          (widget.itemTotalAmount + widget.shippingCost - widget.discount - couponDiscount + widget.tax)){
                      showCustomSnackBar(getTranslated('insufficient_balance', context), context, isToaster: true);
                      } else{

                     await syncOrderProvider.placeSyncWalletOrder(widget.id.toString()).then((value) {
                        if(value.response!=null&&value.response!.statusCode==200){
                         ...
                         showAnimatedDialog(context,  BankTransfer(type: 'order',addressId: int.parse(addressID.toString()),orderId:widget.id.toString() ,), dismissible: true, willFlip: true);

                       }
                       else if(checkout.selectedDigitalPaymentMethodId==5){
                         syncOrderProvider.placeSyncDelayedOrder(widget.id.toString(),addressID.toString(),couponController.text,couponDiscount.toString(),'').then((value) {
      if(value.response!=null&&value.response!.statusCode==200){
...
await syncOrderProvider.placeSyncOrderCashOnDelivery(widget.id.toString(),couponController.text,couponDiscount.toString(),'',addressID).then((value) {
  if(value.response!=null&&value.response!.statusCode==200){
    _callback(true,value.response!.data,'',false);
