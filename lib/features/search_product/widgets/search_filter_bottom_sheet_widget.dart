...
class SearchFilterBottomSheet extends StatefulWidget {
  const SearchFilterBottomSheet({super.key, required this.pagingController});
                  ...
                  return Consumer<BrandController>(
                    builder: (context, brandProvider,_) {
                      return Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
                          ...
                          FilterItemWidget(title: getTranslated('low_to_high_price', context), index: 1),
                          FilterItemWidget(title: getTranslated('high_to_low_price', context), index: 2),
                          FilterItemWidget(title: getTranslated('Low_to_high_quantity', context), index: 3),
                          FilterItemWidget(title: getTranslated('High_to_low_quantity', context), index: 4),
                          FilterItemWidget(title: getTranslated('More_preferable', context), index: 5),
                          FilterItemWidget(title: getTranslated('Highest_ratings', context), index: 6),
                          FilterItemWidget(title: getTranslated('Special_offers', context), index: 7),
        const SizedBox(height: 15,),
        
            Text(getTranslated('Sync_status', context)??'',
                            style: titilliumSemiBold.copyWith(fontSize: Dimensions.fontSizeLarge)),
        
                        SyncFilterItemWidget(title: getTranslated('Sync_status', context), index: 0),
                        SyncFilterItemWidget(title: getTranslated('Asynchronous_products', context), index: 1),
                        SyncFilterItemWidget(title: getTranslated('Synchronized_products', context), index: 2),
...
class SyncFilterItemWidget extends StatelessWidget {
  final String? title;
  final int index;
  const SyncFilterItemWidget({super.key, required this.title, required this.index});
    ...
    return Padding(padding: const EdgeInsets.only(top: Dimensions.paddingSizeDefault),
      child: Container(decoration: BoxDecoration(
        ...
        child: Padding(padding: const EdgeInsets.all(Dimensions.paddingSizeSmall),
          child: Row(children: [
            Expanded(child: Text(title??'', style: textRegular.copyWith(fontSize: Dimensions.fontSizeDefault))),
            InkWell(onTap: ()=> Provider.of<SearchProductController>(context, listen: false).setSyncFilterIndex(index),
                child: Icon(Provider.of<SearchProductController>(context).syncFilterIndex == index? Icons.radio_button_checked: Icons.radio_button_off,
                    color: Provider.of<SearchProductController>(context).syncFilterIndex == index? Theme.of(context).primaryColor: Theme.of(context).hintColor.withOpacity(.15)))
      ],)),),
    );
  }
}
