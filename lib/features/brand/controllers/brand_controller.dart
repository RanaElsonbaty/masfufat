...
class BrandController extends ChangeNotifier {
  ...
  Future< List<BrandModel>> getBrandList(bool reload,int offset) async {
      ApiResponse apiResponse = await brandRepo!.getList(offset: offset);
        _originalBrandList=[];
        apiResponse.response?.data['data'].forEach((brand) => _originalBrandList.add(BrandModel.fromJson(brand)));
        _brandList=[];
        apiResponse.response!.data['data'].forEach((brand) => _brandList.add(BrandModel.fromJson(brand)));
      notifyListeners();
      return _brandList;
