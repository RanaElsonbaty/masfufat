...
class _AllCategoryAndBrandState extends State<AllCategoryAndBrand> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      // appBar:  CustomAppBar(title:'',isBackButtonExist: widget.backButtom!,
      // showResetIcon: true,
      //   // centerTitle: false,
      //   showLogo: true,
      //   reset: Row(children: [
                  ...
                  SizedBox(
                    ...
                    child:BrandListWidget(isHomePage: true, scrollController: ScrollController(),   selectId:selectIndexBrand , selectBrand: (BrandModel val)async{
                       ...
                       if(selectIndexBrand==val.id){
                         ...
                         await category.getBrandCategoryList(val.id);
                         ...
                         scrollController.addListener(_scrollListener);
                         fetchPage(page,true, Provider.of<CategoryController>(Get.context!,listen: false).brandCategoryList.isNotEmpty?Provider.of<CategoryController>(Get.context!,listen: false).brandCategoryList.first.id.toString():'0', Get.context!,true);
                      ...
                      itemBuilder: (context, index) {
                        return InkWell(
                          onTap: (){
                            ...
                            fetchPage(page,false, Provider.of<CategoryController>(context,listen: false).brandCategoryList[selectIndexCategory].id.toString(), Get.context!,true);
                        ...
                        builder:(context, product, child) =>  Consumer<CategoryController>(
                          builder:(context, category, child) =>  Consumer<BrandController>(
                            builder:(context, brand, child) {
                              ...
                              if (selectIndexBrand != null) {
                               for (var element in brand.brandList) {
                                 if(element.id == selectIndexBrand!){
                                   selectedBrandName = element.name;
                              ...
                              child: Row(
                                ...
                                Text('${category.categoryList.isNotEmpty?category.categoryList[selectIndexCategory].name.toString():''} ${selectIndexBrand!=null?'/':''} ${selectIndexBrand!=null?selectBrandName:""}',style: GoogleFonts.cairo(
                                  color: Colors.white,
                                ),),
                                  InkWell(onTap: () {
  ...
  void _scrollListener() async {
    ...
    if (lastPage == false) {
      ...
      if (currentScrollPosition >= maxScrollExtent - 8000.0) {
        ...
        await fetchPage(page,false, Provider.of<CategoryController>(context,listen: false).categoryList[selectIndexCategory].id.toString(), Get.context!,true).then((value) {
          setState(() {
            loading = false;
            _page =(_page+1);
